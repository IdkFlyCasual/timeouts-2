...
function dynamicIntervalCount(cb, delay, amount) {
  ...
  const executeCallback = () => {
    ...
    if (typeof amount === 'number' && count === amount) {
      clearInterval(intv); // Clear interval when count reaches the specified amount
    }
  };

  intv = setInterval(executeCallback, delay);

  // If no amount is provided, return the interval object
